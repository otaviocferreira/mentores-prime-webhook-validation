  ...
  const lessons = options?.emptyCatalog
    ...
    : [
        ...
        },
        {
          id: 'lesson-adv-1',
          level_id: 'level-adv',
          module_id: options?.brokenLessons ? 'module-nao-existe' : 'module-adv-1',
          lesson_code: 'ADV-L1',
          title: 'Threads',
          order_index: 1,
          is_extra: false
        ...
        },
        {
          id: 'lesson-mas-1',
          level_id: 'level-mas',
          module_id: options?.withEmptyNodes ? 'module-mas-1' : 'module-mas-1',
          lesson_code: 'MAS-L1',
          title: 'Mentoria',
          order_index: 1,
          is_extra: false
  ...
  const checkpoints = options?.emptyCatalog
    ...
    : [
        ...
        },
        {
          ...
          level_id: 'level-int',
          module_id: 'module-int-1',
          checkpoint_code: 'INT-C1',
          title: 'Colecoes checkpoint',
          order_index: 1
        }
      ];
  ...
  return {
    ...
    async listPublishedCheckpointsByMentorId(_mentorId: string, levelIds: string[], moduleIds: string[]) {
      return checkpoints.filter(
        (checkpoint) => levelIds.includes(checkpoint.level_id) && moduleIds.includes(checkpoint.module_id)
      );
    },
    ...
    async listLessonProgress(_customerId: string, lessonIds: string[]) {
      return lessonIds.map((lessonId) => ({
        lesson_id: lessonId,
        is_opened: lessonId !== 'lesson-int-1',
        is_completed: lessonId === 'lesson-pro-1'
      }));
    },
    async listCheckpointProgress(_customerId: string, checkpointIds: string[]) {
      return checkpointIds.map((checkpointId) => ({
        ...
        is_opened: true,
        is_completed: checkpointId === 'checkpoint-int-1'
      }));
    },
    async listProjectProgress(_customerId: string, projectIds: string[]) {
      return projectIds.map((projectId) => ({
        project_id: projectId,
        is_opened: projectId === 'project-adv-1',
        is_completed: projectId === 'project-pro-1'
      }));
    }
  };
}
...
test('getMentorCatalog retorna catalogo completo ordenado e com progresso embutido', async () => {
  const response = await getMentorCatalog(
    { mentorSlug: 'java', email: '  ALUNO@example.com  ' },
    createCatalogDeps()
  );

  assert.equal(response.mentor.slug, 'java');
  assert.equal(response.level_filter, null);
  assert.deepEqual(
    response.levels.map((level) => level.code),
    ['INI', 'INT', 'ADV', 'PRO', 'MAS']
  );
  assert.deepEqual(
    response.levels[0]?.modules.map((module) => module.module_code),
    ['INI-01', 'INI-02']
  );
  ...
  assert.equal(response.levels[3]?.modules[0]?.lessons[0]?.progress.is_completed, true);
});
...
test('getMentorCatalog remove modulos sem conteudo e niveis sem modulos restantes', async () => {
  const response = await getMentorCatalog(
    { mentorSlug: 'java', email: 'aluno@example.com' },
    createCatalogDeps({ withEmptyNodes: true })
  );
...
test('getMentorCatalog retorna 400 quando email esta ausente', async () => {
  await assert.rejects(
    () => getMentorCatalog({ mentorSlug: 'java' }, createCatalogDeps()),
    (error: unknown) => {
...
test('getMentorCatalog retorna 404 para customer ausente', async () => {
  await assert.rejects(
    () => getMentorCatalog({ mentorSlug: 'java', email: 'naoexiste@example.com' }, createCatalogDeps()),
    (error: unknown) => {
      assert.ok(error instanceof CatalogError);
...
test('getMentorCatalog retorna 400 para level invalido', async () => {
  await assert.rejects(
    () => getMentorCatalog({ mentorSlug: 'java', email: 'aluno@example.com', level: 'senior' }, createCatalogDeps()),
    (error: unknown) => {
      assert.ok(error instanceof CatalogError);
      assert.equal(error.statusCode, 400);
...
test('getMentorCatalog retorna 404 quando catalogo esta vazio', async () => {
  await assert.rejects(
    () => getMentorCatalog({ mentorSlug: 'java', email: 'aluno@example.com' }, createCatalogDeps({ emptyCatalog: true })),
    (error: unknown) => {
      assert.ok(error instanceof CatalogError);
      assert.equal(error.statusCode, 404);
...
test('getMentorCatalog detecta inconsistencias de parent child', async () => {
  await assert.rejects(
    () => getMentorCatalog({ mentorSlug: 'java', email: 'aluno@example.com' }, createCatalogDeps({ brokenLessons: true })),
    (error: unknown) => {
      assert.ok(error instanceof CatalogError);
